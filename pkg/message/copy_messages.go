...
func CopySelections(ctx *ctx.Context, params ...interface{}) error {
	if len((*ctx).GetState().Selections) == 0 {
		return (*ctx).GetGui().Views.Log.SetLog("Select nothing!!!", view.LogLevel(view.WARNING))
	}
	...
	if err := copyPaths(ctx, paths, (*ctx).GetFileManager().Dir.Path); err != nil {
...
func copyPaths(ctx *ctx.Context, paths []string, dest string) error {
	if err := (*ctx).GetGui().Views.Progress.StartProgress(len(paths)); err != nil {
		return err
	}

	countChan, errChan := (*ctx).GetFileManager().Copy(paths, dest)
	...
	loop:
		...
		if errCount != 0 {
			...
			err = (*ctx).GetGui().Views.Log.SetLog(fmt.Sprintf("Finished to copy %v", paths), view.LogLevel(view.INFO))
		}
